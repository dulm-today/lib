...
// crc 32
static unsigned int s_crc32_table[] =
{
	  0x00000000L,
	  0x04c11db7L,
	  0x09823b6eL,
...
#define CAL_CRC(A)   crc = (crc << 4) ^ s_crc32_table[((crc >> 28) ^ (A >> 4)) & 0x0f],\
	                 crc = (crc << 4) ^ s_crc32_table[((crc >> 28) ^ (A)) & 0x0f]
...
#define crc7_byte(crc,data) crc7_table[( (crc) << 1) ^ (data)]
